...
def main():
    ...
    for seed in range(3):
        ...
        model = BERT(args,device,original_data).to(device)
        trainer = BERTTrainer(args,device,original_data,model)
        original_trainer = copy.deepcopy(trainer)
        print('\nPre-training of BERT4Rec + fine-tuning with FINEST on the original training data\n')
        [original_logits,reference_list,competitive_items, original_perf] = trainer.traintest(original_logits=-1, reference_list=-1,competitive_items=-1, args=args,data = original_data,mode="original_training",seed=seed)
        ...
        print('\nRetraining of BERT4Rec + fine-tuning with FINEST on the perturbed training data\n')
        [temp1,temp2,temp3,perturbed_perf] = original_trainer.traintest(original_logits=original_logits, reference_list=reference_list,competitive_items=competitive_items, args=args,data = perturbed_data,mode="retraining",seed=seed)
