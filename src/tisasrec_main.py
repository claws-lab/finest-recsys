...
def main():
    ...
    parser.add_argument('--fine_tune_lr', default=0.001, type=float, help='learning rate for fine-tuning using FINEST')
    ...
    # initialize the TiSASRec model
    model = TiSASRec(usernum, itemnum, args, device).to(device)    
    for name, param in model.named_parameters():
        try:
            torch.nn.init.xavier_uniform_(param.data)    
        except:  
            pass # just ignore those failed init layersi
    trainer = TiSASRec_Trainer(usernum, itemnum, args,np.random.randint(0,99999999),device,original_dataset, model).to(device)    
    original_trainer = copy.deepcopy(trainer)

    print('\nPre-training of TiSASRec + fine-tuning with FINEST on the original training data\n')
    ...
    print('\nRetraining of TiSASRec + fine-tuning with FINEST on the perturbed training data\n')
